...
open class AnimatedField: UIView {
    ...
    public var type: AnimatedFieldType = .none {
        didSet {
            ...
            if case let AnimatedFieldType.stringpicker(arrStrings,chooseText) = type {
                setupPicker(strings: arrStrings, chooseText: chooseText)
            }
...
//
    ...
    @objc func didChooseDatePicker() {
        ...
        textField.text = date?.format(dateFormat: dateFormat ?? "yyyy / MM / dd")
